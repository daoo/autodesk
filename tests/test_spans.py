...
class TestSpan(unittest.TestCase):
    ...
    def test_span_collect_empty(self):
        spans = list(collect('a', 0, 1, []))
        self.assertEqual(spans, [Span(0, 1, 'a')])

    def test_span_collect_inital(self):
        events = [Event(1, 'a')]
    ...
    def test_span_collect_final(self):
        events = [Event(1, 'a')]
    ...
    def test_span_cut_outside(self):
        spans = [Span(2, 3, 'a')]
        self.assertEqual(list(cut(0, 1, spans)), [])
        self.assertEqual(list(cut(4, 5, spans)), [])

    def test_span_cut_inclusive(self):
        spans = [Span(2, 4, 'a')]
