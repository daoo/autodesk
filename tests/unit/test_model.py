...
def test_build_spans_aggregates_consecutive_identical_states():
    ...
    spans = list(
        build_spans(
            default_state=Session.INACTIVE,
            initial=t1,
            final=t4,
            events=[(t2, Session.ACTIVE), (t3, Session.ACTIVE)],
        ),
    )

    assert spans == [(t1, t2, Session.INACTIVE), (t2, t4, Session.ACTIVE)]
...
def test_build_spans_skips_zero_length_transition_at_initial():
    ...
    spans = list(
        build_spans(
            ...
            initial=t1,
            final=t2,
            events=[(t1, Session.ACTIVE)],
        ),
    )

    assert spans == [(t1, t2, Session.ACTIVE)]
    ...
    with pytest.raises(ValueError, match="events must be monotonic and >= initial"):
        list(
            build_spans(
                ...
                initial=t1,
                final=t3,
                events=[(t2, Session.ACTIVE), (t1, Session.INACTIVE)],
            ),
        )
...
def test_build_spans_invariants_sorted_non_overlapping_non_zero_and_ends_at_final():
    ...
    spans = list(
        build_spans(
            ...
            initial=t1,
            final=t4,
            events=[(t2, Session.ACTIVE), (t3, Session.INACTIVE)],
        ),
    )
...
def test_get_session_state_empty(mocker):
    ...
    assert model.get_session_state() == Session.INACTIVE


def test_get_desk_state_empty(mocker):
    model = Model(empty_data_store(mocker))
    assert model.get_desk_state() == Desk.DOWN
...
def test_get_active_time_active_zero(mocker):
    t = datetime(2018, 1, 1)
    model = Model(
        fake_data_store(mocker, session_events=[(t, Session.ACTIVE)], desk_events=[])
    )
...
def test_get_active_time_active_for_10_minutes(mocker):
    ...
    model = Model(
        fake_data_store(mocker, session_events=[(t1, Session.ACTIVE)], desk_events=[]),
    )
...
def test_get_active_time_just_after_desk_change(mocker):
    ...
    model = Model(
        fake_data_store(
            mocker, session_events=[(t1, Session.ACTIVE)], desk_events=[(t2, Desk.UP)]
        ),
    )
...
def test_get_active_time_ignores_duplicate_desk_state_events(mocker):
    ...
    model = Model(
        fake_data_store(
            mocker,
            session_events=[(t1, Session.ACTIVE)],
            desk_events=[(t2, Desk.UP), (t3, Desk.UP)],
        ),
    )
...
def test_compute_hourly_count_active_30_minutes(mocker):
    ...
    model = Model(
        fake_data_store(
            mocker,
            session_events=[(t1, Session.ACTIVE), (t2, Session.INACTIVE)],
            desk_events=[],
        ),
    )
...
def test_compute_hourly_count_active_0_minutes(mocker):
    ...
    model = Model(
        fake_data_store(
            mocker, session_events=[(t1, Session.INACTIVE)], desk_events=[]
        ),
    )
    result = model.compute_hourly_count(t1, t2)
...
def test_set_session_state_active(inmemory_model):
    ...
    inmemory_model.set_session(t1, Session.ACTIVE)

    assert inmemory_model.get_session_state() == Session.ACTIVE
...
def test_set_session_state_inactive(inmemory_model):
    ...
    inmemory_model.set_session(t1, Session.INACTIVE)

    assert inmemory_model.get_session_state() == Session.INACTIVE
    assert inmemory_model.datastore.get_session_events_between(t1, t1) == [
        (t1, Session.INACTIVE),
    ]
...
def test_set_desk_state_up(inmemory_model):
    ...
    inmemory_model.set_desk(t1, Desk.UP)

    assert inmemory_model.get_desk_state() == Desk.UP
    assert inmemory_model.datastore.get_desk_events_between(t1, t1) == [(t1, Desk.UP)]


def test_set_desk_state_down(inmemory_model):
    t1 = datetime(2018, 1, 1)

    inmemory_model.set_desk(t1, Desk.DOWN)

    assert inmemory_model.get_desk_state() == Desk.DOWN
    assert inmemory_model.datastore.get_desk_events_between(t1, t1) == [(t1, Desk.DOWN)]
